...
final class AppKitComponentTests: XCTestCase {
    ...
    /// Exhaustive reproduction: split (with search) → focused → search → check ALL state
    func testReproduceSplitSearchThenFocusedSearchLayoutCorruption() throws {
        let renderer = try makeRendererOrSkip()
        let fullWidth: CGFloat = 800
        let height: CGFloat = 400

        let window = NSWindow(
            contentRect: NSRect(x: 0, y: 0, width: fullWidth, height: height),
        ...
        let hostView = NSView(frame: window.contentView!.bounds)
        hostView.autoresizingMask = [.width, .height]
        window.contentView?.addSubview(hostView)

        // Create two controllers with data
        let controller1 = TerminalController(
            rows: 20, cols: 80,
            termEnv: "xterm-256color", textEncoding: .utf8,
            scrollbackInitialCapacity: 65536, scrollbackMaxCapacity: 65536,
            fontName: "Menlo", fontSize: 13
        )
        let controller2 = TerminalController(
            rows: 20, cols: 80,
        ...
        // === Step 1: Split view with 2 terminals ===
        let splitView = SplitTerminalContainerView(
        ...
        let splitCols = controller1.withModel { $0.cols }

        // Add data to scrollback (simulating real terminal usage in split view)
        let cellX = Cell(codepoint: 88, attributes: .default, width: 1, isWideContinuation: false)
        for _ in 0..<100 {
            let row = (0..<splitCols).map { _ in cellX }
            controller1.scrollback.appendRow(ArraySlice(row), isWrapped: false)
        }

        // Find the TerminalScrollViews inside the split container
        let splitScrollViews = allSubviews(in: splitView).compactMap { $0 as? TerminalScrollView }
        XCTAssertEqual(splitScrollViews.count, 2, "Expected 2 scroll views in split")

        // === Step 2: Search in first terminal (in split view) ===
        splitScrollViews[0].terminalView.beginSearch()
        controller1.withModel { $0.grid.setCell(cellX, at: 0, col: 0) }
        _ = splitScrollViews[0].terminalView.updateSearch(query: "X")
        splitScrollViews[0].terminalView.endSearch()

        // === Step 3: Search in second terminal (in split view) ===
        splitScrollViews[1].terminalView.beginSearch()
        controller2.withModel { $0.grid.setCell(cellX, at: 0, col: 0) }
        _ = splitScrollViews[1].terminalView.updateSearch(query: "X")
        splitScrollViews[1].terminalView.endSearch()

        // === Step 4: Cmd+Click → focused (maximize second terminal) ===
        splitView.detachControllersForPresentationTransition()
        splitView.removeFromSuperview()

        let focusedSV = TerminalScrollView(frame: hostView.bounds, renderer: renderer)
        focusedSV.autoresizingMask = [.width, .height]
        hostView.addSubview(focusedSV)
        focusedSV.terminalView.terminalController = controller2
        focusedSV.terminalView.syncScaleFactorIfNeeded()
        window.makeFirstResponder(focusedSV.terminalView)
        hostView.layoutSubtreeIfNeeded()

        let focusedCols = controller2.withModel { $0.cols }
        XCTAssertGreaterThan(focusedCols, splitCols, "Focused cols (\(focusedCols)) must be > split cols (\(splitCols))")

        // Pre-search state: everything should be full width
        let preClipW = focusedSV.contentView.bounds.width
        let preTVW = focusedSV.terminalView.bounds.width
        XCTAssertEqual(preClipW, fullWidth, "Pre-search clip width must be full")
        XCTAssertEqual(preTVW, fullWidth, "Pre-search TV bounds width must be full")

        // === Step 5: Open search bar (Cmd+F) ===
        focusedSV.terminalView.beginSearch()

        // === Step 6: Type a search keyword ===
        _ = focusedSV.terminalView.updateSearch(query: "X")
        hostView.layoutSubtreeIfNeeded()

        // Force a draw cycle to trigger syncScroller (mimics real app display link)
        focusedSV.terminalView.draw(focusedSV.terminalView.bounds)
        focusedSV.syncScroller()
        hostView.layoutSubtreeIfNeeded()

        // === Step 7: Capture render snapshot and check EVERY dimension ===
        let snapshot = controller2.captureRenderSnapshot()
        let postClipW = focusedSV.contentView.bounds.width
        let postDocW = focusedSV.documentView!.frame.width
        let postTVBounds = focusedSV.terminalView.bounds
        let postTVFrame = focusedSV.terminalView.frame
        let postDrawableW = focusedSV.terminalView.drawableSize.width

        var failures: [String] = []

        if snapshot.cols != focusedCols {
            failures.append("snapshot.cols=\(snapshot.cols) expected=\(focusedCols)")
        }
        if postClipW != fullWidth {
            failures.append("clipView.width=\(postClipW) expected=\(fullWidth)")
        }
        if postDocW != fullWidth && postDocW < fullWidth {
            failures.append("docView.width=\(postDocW) expected>=\(fullWidth)")
        }
        if postTVBounds.width != fullWidth {
            failures.append("TV.bounds.width=\(postTVBounds.width) expected=\(fullWidth)")
        }
        if postTVFrame.width != fullWidth {
            failures.append("TV.frame.width=\(postTVFrame.width) expected=\(fullWidth)")
        }

        // Check ALL visible rows have the correct number of cells
        for (i, row) in snapshot.visibleRows.enumerated() {
            if row.cells.count != snapshot.cols {
                failures.append("visibleRow[\(i)].cells.count=\(row.cells.count) expected=\(snapshot.cols)")
            }
        }

        // Check the scrollback rows that are visible (if scrolled up via revealSearchMatch)
        let scrollOffset = controller2.scrollOffset
        if scrollOffset > 0 {
            failures.append("NOTE: scrollOffset=\(scrollOffset) — viewing scrollback rows")
        }

        if !failures.isEmpty {
            XCTFail("Layout corruption detected after split→focused→search:\n" + failures.joined(separator: "\n"))
        }

        focusedSV.terminalView.endSearch()
    }

    func testSearchMatchMapIsChildOfTerminalViewNotScrollView() throws {
        let renderer = try makeRendererOrSkip()
        let scrollView = TerminalScrollView(frame: NSRect(x: 0, y: 0, width: 320, height: 240), renderer: renderer)
        let view = scrollView.terminalView!
        let controller = TerminalController(
        ...
        view.beginSearch()
        let map = scrollView.searchMatchMapView
        XCTAssertNotNil(map)
        // Map must be a subview of TerminalView, NOT of the scroll view itself,
        // to avoid interfering with NSScrollView's layout of clip view / content view.
        XCTAssertTrue(map!.superview === view,
            "SearchMatchMapView must be a child of TerminalView, not TerminalScrollView")

        view.endSearch()
    }

    func testSearchAfterSplitToFocusedTransitionPreservesFullWidthCols() throws {
        let renderer = try makeRendererOrSkip()

        // Step 1: Simulate split view — controller at half width
        let halfWidth: CGFloat = 320
        let fullWidth: CGFloat = 640
        ...
        // Assign to a half-width view (simulating split view)
        let splitScrollView = TerminalScrollView(frame: NSRect(x: 0, y: 0, width: halfWidth, height: height), renderer: renderer)
        splitScrollView.terminalView.terminalController = controller
        let splitCols = controller.withModel { $0.cols }

        // Step 2: Detach and assign to full-width view (simulating focused view)
        splitScrollView.terminalView.detachControllerForPresentationTransition()

        let focusedScrollView = TerminalScrollView(frame: NSRect(x: 0, y: 0, width: fullWidth, height: height), renderer: renderer)
        focusedScrollView.terminalView.terminalController = controller
        let focusedCols = controller.withModel { $0.cols }

        XCTAssertGreaterThan(focusedCols, splitCols,
            "Focused view must have more cols than split view")

        // Step 3: Begin search — this must NOT change cols
        focusedScrollView.terminalView.beginSearch()
        let cellA = Cell(codepoint: 65, attributes: .default, width: 1, isWideContinuation: false)
        controller.withModel { model in
            model.grid.setCell(cellA, at: 0, col: 0)
        }
        _ = focusedScrollView.terminalView.updateSearch(query: "A")

        let colsAfterSearch = controller.withModel { $0.cols }
        XCTAssertEqual(colsAfterSearch, focusedCols,
            "Search must not change terminal cols — was \(focusedCols), now \(colsAfterSearch)")

        // Step 4: Verify the scroll view's clip view width matches full width
        let clipWidth = focusedScrollView.contentView.bounds.width
        XCTAssertEqual(clipWidth, fullWidth,
            "Clip view width must remain full width during search")

        focusedScrollView.terminalView.endSearch()
    }

    /// Exhaustive reproduction test: real SplitTerminalContainerView → focused → search.
    func testSplitToFocusedSearchDoesNotCorruptScrollViewLayout() throws {
        let renderer = try makeRendererOrSkip()
        let fullWidth: CGFloat = 800
        let height: CGFloat = 400
        ...
        // Step 1: Create two controllers and a real SplitTerminalContainerView
        let controller1 = TerminalController(
            rows: 20, cols: 80,
            termEnv: "xterm-256color", textEncoding: .utf8,
            scrollbackInitialCapacity: 4096, scrollbackMaxCapacity: 4096,
        ...
        // Step 2: Transition to focused — mimic switchToFocused exactly
        splitView.detachControllersForPresentationTransition()
        splitView.removeFromSuperview()

        let focusedSV = TerminalScrollView(
            frame: hostView.bounds,
            renderer: renderer
        )
        focusedSV.autoresizingMask = [.width, .height]
        hostView.addSubview(focusedSV)
        focusedSV.terminalView.terminalController = controller1
        focusedSV.terminalView.syncScaleFactorIfNeeded()
        hostView.layoutSubtreeIfNeeded()

        let focusedCols = controller1.withModel { $0.cols }
        XCTAssertGreaterThan(focusedCols, splitCols,
            "Focused must have more cols (\(focusedCols)) than split (\(splitCols))")

        // Record pre-search state
        let preClipWidth = focusedSV.contentView.bounds.width
        let preDocWidth = focusedSV.documentView!.frame.width
        let preTVWidth = focusedSV.terminalView.bounds.width
        let preDrawableW = focusedSV.terminalView.drawableSize.width

        // Add scrollback data to make documentView tall (mimics real terminal usage)
        let cellA = Cell(codepoint: 65, attributes: .default, width: 1, isWideContinuation: false)
        for _ in 0..<500 {
            controller1.scrollback.appendRow(ArraySlice([cellA]), isWrapped: false)
        }
        focusedSV.syncScroller()
        hostView.layoutSubtreeIfNeeded()
        ...
        // Step 3: Search
        focusedSV.terminalView.beginSearch()
        controller1.withModel { $0.grid.setCell(cellA, at: 0, col: 0) }
        _ = focusedSV.terminalView.updateSearch(query: "A")
        hostView.layoutSubtreeIfNeeded()

        // Step 4: Exhaustive verification
        let postCols = controller1.withModel { $0.cols }
        let postClipW = focusedSV.contentView.bounds.width
        let postDocW = focusedSV.documentView!.frame.width
        let postTVBounds = focusedSV.terminalView.bounds
        let postTVFrame = focusedSV.terminalView.frame
        ...
        XCTAssertEqual(postCols, focusedCols, "cols changed: \(focusedCols)→\(postCols)")
        XCTAssertEqual(postClipW, preClipWidth, "clipView width changed: \(preClipWidth)→\(postClipW)")
        XCTAssertEqual(postDocW, preDocWidth, "docView width changed: \(preDocWidth)→\(postDocW)")
        XCTAssertEqual(postTVBounds.width, preTVWidth, "TV bounds width changed: \(preTVWidth)→\(postTVBounds.width)")
        XCTAssertEqual(postTVFrame.width, fullWidth, "TV frame width (\(postTVFrame.width)) != fullWidth (\(fullWidth))")
        XCTAssertEqual(postDrawableW, preDrawableW, "drawableSize.width changed: \(preDrawableW)→\(postDrawableW)")

        // Check no rogue subviews on scroll view
        let scrollViewDirectSubviews = focusedSV.subviews.filter { !($0 is NSClipView) && !($0 is NSScroller) }
        for sub in scrollViewDirectSubviews {
            XCTAssertFalse(sub is SearchMatchMapView,
                "SearchMatchMapView must NOT be a direct child of the scroll view")
        }

        focusedSV.terminalView.endSearch()
    }

    func testRenderSnapshotPadsScrollbackRowsToCurrentColCount() {
        let renderer = try! makeRendererOrSkip()
        let controller = TerminalController(
            rows: 3, cols: 20,
            termEnv: "xterm-256color", textEncoding: .utf8,
        ...
        // Append a scrollback row with only 10 cells (simulating a row stored at narrower width)
        let narrowCells = (0..<10).map {
            Cell(codepoint: UInt32(65 + $0), attributes: .default, width: 1, isWideContinuation: false)
        }
        controller.scrollback.appendRow(ArraySlice(narrowCells), isWrapped: false)

        // Now the grid has cols=20 but the scrollback row has only 10 cells.
        // Scroll up to make the scrollback row visible.
        controller.scrollUp(lines: 1)

        let snapshot = controller.captureRenderSnapshot()
        // The first visible row should be the scrollback row, padded to cols=20
        guard let firstRow = snapshot.visibleRows.first else {
            XCTFail("No visible rows in snapshot")
            return
        }
        XCTAssertEqual(firstRow.cells.count, snapshot.cols,
            "Scrollback row must be padded to current cols (\(snapshot.cols)), got \(firstRow.cells.count)")
    }
